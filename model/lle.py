...
class MobileIELLENet(nn.Module):
    def __init__(self, channels, rep_scale=4):
        ...
        self.tail = MBRConv3(channels, 4, rep_scale=rep_scale)  # 4通道: 1光照 + 3去噪残差
    ...
    def forward(self, x):
        ...
        # 带残差去噪的 Retinex
        out = self.tail(x4)
        illu_map = torch.sigmoid(out[:, 0:1, :, :]) * 0.99 + 0.01  # 第1通道: 光照图
        noise_res = out[:, 1:4, :, :]  # 后3通道: 去噪残差
        # 除法恢复亮度 + 残差抵消放大的噪声
    ...
    def forward_warm(self, x):
        ...
        # 带残差去噪的 Retinex
        out1 = self.tail(x)
        illu1 = torch.sigmoid(out1[:, 0:1, :, :]) * 0.99 + 0.01
        res1 = out1[:, 1:4, :, :]
        enhanced1 = (x_in / illu1) + res1
        
        out2 = self.tail_warm(x)
        illu2 = torch.sigmoid(out2[:, 0:1, :, :]) * 0.99 + 0.01
        res2 = out2[:, 1:4, :, :]
...
class MobileIELLENetS(nn.Module):
    def __init__(self, channels):
        ...
        self.tail = nn.Conv2d(channels, 4, 3, 1, 1)  # 4通道: 1光照 + 3去噪残差
    ...
    def forward_warm(self, x):
        ...
        # 带残差去噪的 Retinex
        out = self.tail(x)
        illu_map = torch.sigmoid(out[:, 0:1, :, :]) * 0.99 + 0.01
        noise_res = out[:, 1:4, :, :]
        enhanced = (x_in / illu_map) + noise_res
